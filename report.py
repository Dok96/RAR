...
from config import plc_ip,slot,rack
...
def main_task(plc_manager):
    ...
    while not stop_event.is_set():  # Проверяем флаг
        print(f"Выполняется задача {counter}")
        counter += 1
        time.sleep(2)  # Имитация работы (например, чтение с ПЛК)
...
if __name__ == "__main__":
    ...
    plc_manager = PlcManager(plc_ip, rack, slot)
